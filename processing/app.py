...
def populate_stats():
    ...
    num_negative_tests = len(negative_tests) + results["num_negative_tests"]
    ...
    logger.debug(f'Total number of negative tests: {num_negative_tests}. TraceID: {trace_id}')
    logger.debug(f'Age group with the most positive cases{highest_positive_occuring_age}. TraceID: {trace_id}')

    session = DB_SESSION()
    stat = Stats(num_tests_results, num_users, num_positive_tests, num_negative_tests, highest_positive_occuring_age, current_time)
